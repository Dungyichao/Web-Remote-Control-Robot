...
int send_message(int fd, char image_path[], const char head[]);
//char *str_timestamp();
...
void handle_client(client_t *client) {
    ...
    if (result < 0) { //failed to read client request
    ...
    if(strlen(client->uri) == 1 && 1==2){
        char path_head[500] = ".";
        char *copy_head = (char *)malloc(strlen(RESPONSE_OK_HEADER_TEMPLATE) +200);
        strcat(path_head, "/index.html");
        strcat(copy_head, "Content-Type: text/html\r\n\r\n");
        send_message(client->stream_fd, path_head, RESPONSE_OK_HEADER_TEMPLATE);
...
int read_request(client_t *client){
    ...
    while (running) {
        ...
        if (size < 0) {
            if (errno == EAGAIN) { //timeout reading from client
    ...
    while (running && (line_end = strstr(buf + offs, "\r\n"))) {
        if (offs == 0) { /* first request line */
            ...
            //sscanf will retrieve data from buf into client->method, client->uri, client->http_ver
            if (found != 3) {
                //ERROR_CLIENT(client, "invalid request line");
                printf("invalid request line from client %s:%d \n", client->addr, client->port);
                return -1;
            }
            ...
            if (header_mid && header_mid < line_end) {
                ...
                //copy at most (header_mid - buf - offs) bytes from char starting from buff+offs pointer into header_name
                header_mid += 1; /* skip ":" */
    ...
    //printf("client buff: %s", buf);
    printf("client method: %s, client uri: %s, client method: %s \n", client->method, client->uri, client->http_ver);
